"""
Based on https://github.com/wujianguo/openai-proxy/blob/main/main.py
"""
...
class LlmProxy(r.BaseJobExecutor):
    def __init__(self, url='https://api.openai.com/v1/'):
        super().__init__('llmproxy')
        self.url = url
        self.define_vars('url')

    def setupRestApp(self, app):
        @app.route('/v1/', defaults={'path': ''})
        @app.route('/v1/<path:path>', methods=['GET', 'POST', 'PUT', 'DELETE'])
        def proxy(path):
            url = request.url.replace(request.host_url, self.url)
            stream = None
            try:
